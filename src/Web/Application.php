...
class Application extends \H1Soft\H\HApplication {
	...
	public static function checkController($_name) {		
		self::$_router->strNameSpace = sprintf("\%s\%s\Controller\%s",Application::app()->src,
			self::$_router->getAppName(),$_name);
		$controller = self::$_router->getControllerNameSpace();
		if(is_file(self::$appsPath.sprintf("%s/Controller/%s.php",self::$_router->getAppName(),$_name)) 
			&& class_exists(self::$_router->strNameSpace)){
